...
public class E02_Start {
	...
	public static void main(String[] args) {
		...
		Grupa jp27 = kreirajGrupu(1, "JP27", 20, new Date(),
				kreirajSmjer(1, "Java programiranje", 5999.99f, 500, true));

		Grupa pp26 = kreirajGrupu(1, "PP26", 20, new Date(),
				kreirajSmjer(1, "PHP programiranje", 5999.99f, 500, false));
		...
		racuni[0] = kreirajRacun(new Date(), 871.61, kreirajKupca("Marko Marulić", "Julija Klovića 12"));
		racuni[1] = kreirajRacun(new Date(), 757.20, kreirajKupca("Ivan Odobašić", "Marina Držica 44"));
		racuni[2] = kreirajRacun(new Date(), 211.71, kreirajKupca("Martin Crnković", "Anastasa Popovića 81"));

		System.out.println(racuni[1].getKupac().getAdresa());
	...
	private static Grupa kreirajGrupu(int sifra, String naziv, int mp, Date datum, Smjer s) {
		Grupa g = new Grupa();
		g.setSifra(sifra);
		g.setNaziv(naziv);
